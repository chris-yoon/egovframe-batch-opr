...
<%
 /**
  * @Class Name : EgovBatchDlbrtConfmManage.jsp
  * @Description : 배치심의 관리 화면
...
function fncSelectBatchDlbrtList(pageNo){
	...
	if(flag==0){
	    ...
	    document.listForm.action = "<c:url value='/bopr/bam/EgovBatchDlbrtConfmList.do'/>";
...
function fncSelectBatchDlbrt(batchDlbrt) {
    ...
    document.listForm.action = "<c:url value='/bopr/bam/EgovBatchDlbrtConfm.do'/>";
    document.listForm.submit();     
}
...
function fncBatchDlbrtDeleteList() {
    ...
    if(fncManageChecked()) {	
        document.listForm.action = "<c:url value='/bopr/bam/EgovBatchDlbrtConfmListDelete.do'/>";
        document.listForm.submit();
    }
}

function fncAddBatchDlbrtView() {
    document.listForm.action = "<c:url value='/bopr/bam/EgovBatchDlbrtConfmUpdate.do'/>";
    document.listForm.submit();     
}
...
<form name="listForm" method="post" action="<c:url value='/bopr/bam/EgovBatchDlbrtConfmList.do'/>">
<div class="contsBody">
	<h2>배치심의 관리</h2>
	<div class="location">배치심의 > 배치심의 관리 > <strong>목록</strong></div>
	...
	<div class="search">
		<fieldset class="searchboxA">
			...
			<select id="searchCondition" name="searchCondition" class="serSel" style="" title="검색조건 선택" onchange="javascript:fncChangeCondition();return false;">
				...
				<option value="1" <c:if test="${dlbrtVO.searchCondition == '1'}"> selected </c:if>>배치심의번호</option>    
				<option value="2" <c:if test="${dlbrtVO.searchCondition == '2'}"> selected </c:if>>배치명</option>       
			</select>
			<label for="searchKeyword" class="disp_none">검색어 입력</label>
			<input id="searchKeyword" name="searchKeyword" class="inptext" title="검색어 입력" type="text" onkeyup="press();return false;" 
	...
	<div class="Btn">
		<span class="bbsBtn"><a title="배치심의건 삭제" href="javascript:fncBatchDlbrtDeleteList()">삭제</a></span>
	</div>

	<div class="bbsList">
		<table  summary="배치명, 배치심의번호, 업무심의명, 등록일시, 승인여부 등의 배치심의 목록입니다.">
		<caption>배치심의 목록</caption>
		<colgroup>
			<col style="width:5%" >
			<col style="width:15%" >
			<col style="width:10%" >
		...
		<tr>
			<td><input title="배치심의 선택" type="checkbox" name="delYn" class="check2"><input type="hidden" name="checkId" value="<c:out value="${batchDlbrt.batchDlbrtNo}"/>" /></td>
		    ...
		    <td><c:out value="${batchDlbrt.frstRegistPnttm}"/></td>
		    <td class="center">
