...
<%
 /**
  * @Class Name : EgovJobDlbrtConfmManage.jsp
  * @Description : 업무심의 관리 화면
...
function fncSelectJobDlbrtList(pageNo){
	...
	if(flag==0){
	    ...
	    document.listForm.action = "<c:url value='/bopr/bam/EgovJobDlbrtConfmList.do'/>";
...
function fncSelectJobDlbrt(jobDlbrt) {
    ...
    document.listForm.action = "<c:url value='/bopr/bam/EgovJobDlbrtConfm.do'/>";
    document.listForm.submit();     
}
...
function fncJobDlbrtDeleteList() {
    ...
    if(fncManageChecked()) {	
        document.listForm.action = "<c:url value='/bopr/bam/EgovJobDlbrtConfmListDelete.do'/>";
        document.listForm.submit();
    }
}

function fncAddJobDlbrtView() {
    document.listForm.action = "<c:url value='/bopr/bam/EgovJobDlbrtConfmUpdate.do'/>";
    document.listForm.submit();     
}
...
<form name="listForm" action="<c:url value='/bopr/bam/EgovJobDlbrtConfmList.do'/>" method="post">
<div class="contsBody">
	<h2>업무심의 관리</h2>
	<div class="location">배치심의 > 업무심의 관리 > <strong>목록</strong></div>
	
	<!-- 검색영역 -->
	<div class="search">
		<fieldset class="searchboxA">
			<legend>검색 영역</legend>
			<label for="searchSeCode" class="disp_none">업무구분 검색조건 선택</label>
			<select id="searchSeCode" name="searchSeCode" class="serSel" style="" title="업무구분 검색조건 선택">
				...
				<c:forEach var="cmmCode" items="${cmmCode}" varStatus="status">
		    	 	<option value='<c:out value="${cmmCode.code}"/>' <c:if test="${dlbrtVO.searchSeCode == cmmCode.code}"> selected </c:if>><c:out value="${cmmCode.codeNm}"/>
			    </c:forEach>
			</select>
			<label for="searchResultCode" class="disp_none">승인여부 검색조건 선택</label>
			<select id="searchResultCode" name="searchResultCode" class="serSel" style="" title="승인여부 검색조건 선택">
				<option value="">승인여부선택</option>
				<option value="NU" <c:if test="${dlbrtVO.searchResultCode == 'NU'}"> selected </c:if>>미승인</option>
				<option value="CN" <c:if test="${dlbrtVO.searchResultCode == 'CN'}"> selected </c:if>>승인</option>
				<option value="RT" <c:if test="${dlbrtVO.searchResultCode == 'RT'}"> selected </c:if>>반려</option>
				<option value="RE" <c:if test="${dlbrtVO.searchResultCode == 'RE'}"> selected </c:if>>재요청</option>
			</select>
			...
			<select id="searchCondition" name="searchCondition" class="serSel" style="" title="검색조건 선택" onchange="javascript:fncChangeCondition();">
				...
				<option value="1" <c:if test="${dlbrtVO.searchCondition == '1'}"> selected </c:if>>업무심의번호</option>    
				<option value="2" <c:if test="${dlbrtVO.searchCondition == '2'}"> selected </c:if>>업무심의제목</option>       
			</select>
			...
			<input id="searchKeyword" name="searchKeyword" class="inptext" title="검색어 입력" type="text" 
				<c:if test="${dlbrtVO.searchKeyword ne ''}">value="<c:out value='${dlbrtVO.searchKeyword}'/>"</c:if>
				<c:if test="${dlbrtVO.searchKeyword eq '' && dlbrtVO.searchCondition eq ''}">value="검색조건을 선택하세요" disabled="disabled"</c:if>
			onkeyup="press();" />
			&nbsp;&nbsp;등록일자&nbsp;
			<label for="searchKeywordFrom" class="disp_none">검색 시작일자 입력</label>
			<input name="searchKeywordFrom" readonly type="text" title="검색 시작일자" size="10" value="<c:out value="${dlbrtVO.searchKeywordFrom}"/>"  />
			...
			<input name="searchKeywordTo" readonly type="text" title="검색 마지막일자" size="10" value="<c:out value="${dlbrtVO.searchKeywordTo}"/>"  />
	...
	<div class="Btn">
		<span class="bbsBtn"><a title="업무심의건 삭제" href="javascript:fncJobDlbrtDeleteList()">삭제</a></span>
	</div>

	<div class="bbsList">
		<table  summary="업무심의제목, 업무심의번호, 업무구분, 등록일시, 승인여부 등의 업무심의 목록입니다.">
		<caption>업무심의 목록</caption>
		<colgroup>
			<col style="width:5%" >
			<col style="width:15%" >
			<col style="width:15%" >
		...
		<tr>
			<td><input type="checkbox" title="업무심의 선택" name="delYn" class="check2"><input type="hidden" name="checkId" value="<c:out value="${jobDlbrt.jobDlbrtNo}"/>" /></td>
