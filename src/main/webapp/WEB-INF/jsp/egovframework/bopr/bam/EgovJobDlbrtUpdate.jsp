...
<c:set var="updaterFlag" value="${empty jobDlbrt.jobDlbrtResultCode ? 'UPDATE' : ((jobDlbrt.jobDlbrtResultCode=='RE' || reconfirm=='Y') ? 'RECONFIRM' : 'DISABLE')}"/>
...
function fncJobDlbrtUpdate() {
	...
	var varFrom = document.getElementById("jobDlbrtManage");
	varFrom.action = "<c:url value='/bopr/bam/EgovJobDlbrtUpdate.do'/>";
...
<div class="contsBody">
	<h2>업무심의요청</h2>
	<div class="location">배치심의 > 업무심의요청 > <strong>수정</strong></div>

	<div class="Btn">
		<span class="bbsBtn"><a title="업무심의요청 목록" href="javascript:fncSelectJobDlbrtList()">목록</a></span>
		<c:if test="${updaterFlag == 'UPDATE'}">
		<span class="bbsBtn"><a title="업무심의요청 수정내용 저장" href="javascript:fncJobDlbrtUpdate()">저장</a></span>
	...
	<div class="bbsDetail">
		<table  summary="업무구분, 업무심의제목, 업무심의내용, 첨부파일, 비고 , 등록수정 정보 등의업무심의요청관리 상세정보입니다.">
		...
		<tr>
			...
			<select name="jobSeCode" id="jobSeCode" title="업무 구분 선택" <c:if test="${updaterFlag == 'DISABLE'}">disabled="disabled"</c:if>>
		...
		<tr>
			...
			<input name="jobDlbrtNm" id="jobDlbrtNm" title="업무심의제목" type="text" value="<c:out value='${jobDlbrt.jobDlbrtNm}'/>" <c:if test="${updaterFlag == 'DISABLE'}">readonly="readonly"</c:if> maxLength="50" size="50" /></td>
		</tr>
		<tr>
			<th>업무심의내용</th>
			<td>
			<label for="jobDlbrtCn" class="disp_none">업무심의내용</label>
			<textarea name="jobDlbrtCn" title="업무심의내용"  <c:if test="${updaterFlag == 'DISABLE'}">readonly="readonly"</c:if> id="jobDlbrtCn" rows="5" cols="80"><c:out value="${jobDlbrt.jobDlbrtCn}"/></textarea></td>
		</tr>
		...
		<tr>
		  ...
		  <tr>  
		    ...
		    <td nowrap>
				...
				<c:if test="${updaterFlag == 'UPDATE' || updaterFlag == 'RECONFIRM'}">
		...
		<tr>
			...
			<textarea name="remark" id="remark" title="비고"  <c:if test="${updaterFlag == 'DISABLE'}">readonly="readonly"</c:if> rows="5" cols="80"><c:out value="${jobDlbrt.remark}"/></textarea></td>
		</tr>
		<tr>
			<th>최초등록자ID</th>
			<td>
...
<!-- 첨부파일용 -->
	...
	<c:if test="${jobDlbrt.atchFileId ne null || jobDlbrt.atchFileId ne ''}"> 						
...
<input type="hidden" name="returnUrl" value="<c:url value='/bopr/bam/EgovJobDlbrtUpdateView.do'/>"/>
