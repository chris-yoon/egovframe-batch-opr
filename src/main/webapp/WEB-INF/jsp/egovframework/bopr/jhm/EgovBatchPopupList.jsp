...
<%
 /**
  * @Class Name : EgovBatchPopupList.jsp
  * @Description : 배치 배포 완료된 것 팝업 조회
  * @Modification Information
  * @
  * @  수정일           수정자          수정내용
  * @ -------    --------  ---------------------------
  * @ 2012.09.14 유현웅           최초 생성
  *
  *  @author 유현웅
  *  @since 2012.09.14
  *  @version 1.0 
...
function fncSelectBatchDlbrtList(pageNo){
	...
	if(flag==0){
	    ...
	    document.listForm.action = "<c:url value='/bopr/bam/EgovBatchPopupList.do'/>";
...
function fncSelectBatchDlbrt(batchDlbrt) {
    opener.document.listForm.searchKeyword.value = batchDlbrt;
    self.close();
}
...
<form name="listForm" action="<c:url value='/bopr/bam/EgovBatchPopupList.do'/>" method="post">
<div class="popupBody">
	<h2>배치 팝업</h2>
	<div class="location">배치 팝업 > <strong>목록</strong></div>
		...
		<fieldset class="searchboxA">
			...
			<select id="searchSeCode" name="searchSeCode" class="serSel" style="" title="업무구분 검색조건 선택">
				...
				<c:forEach var="cmmCode" items="${cmmCode}" varStatus="status">
		    	 	<option value='<c:out value="${cmmCode.code}"/>' <c:if test="${batchDlbrtVO.searchSeCode == cmmCode.code}"> selected </c:if>><c:out value="${cmmCode.codeNm}"/>
			    </c:forEach>
			</select>
	...
	<div class="bbsList">
		<table  summary="업무구분, 배치ID, 배치명 등의 업무심의 팝업목록입니다.">
		...
		<tr>
		    ...
		    <td><c:out value="${batchDlbrt.batchId}"/></td>
		    <td><a href="javascript:fncSelectBatchDlbrt('<c:out value="${batchDlbrt.batchNm}"/>')"><c:out value="${batchDlbrt.batchNm}"/></a></td>
		</tr>
		</c:forEach>
		</tbody>
		</table>
