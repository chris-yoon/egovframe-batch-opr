...
<!--
    ...
    */
    ...
    /*
        스케줄 목록 상세 조회 화면 호출
    */
    function fncSelect(batchId, schdulNo)
    {
        objParam.batchId.value = batchId;
        objParam.schdulNo.value = schdulNo;
        
        objParam.action = "<c:url value='/bopr/sim/selectSchdul.do'/>";
        objParam.submit();
    }
...
<form id="parameter" name="parameter" method="post" action="">
    ...
    <div class="contsBody">
        ...
        <!-- Title Start -->
        <h2>스케줄관리</h2>
        <div class="location">배치정보 &gt; 스케줄관리 &gt; <strong>목록</strong></div>
        <!-- Title End -->
        ...
        <div class="search">
            <fieldset class="searchboxA">
                ...
                <a href="#LINK" onClick="fn_egov_NormalCalendar(parameter,'', parameter.txtExecutSchdulDe);" title="실행일자 조회 팝업, 새창열림">
                    <img name="calendarImg" src="<c:url value='/images/egovframework/com/cmm/icon/bu_icon_carlendar.gif' />"  align="middle" style="border:0px" alt="실행일자 조회 팝업, 새창열림">
                </a>
        ...
        <!-- 버튼 Start -->
        <div class="Btn">
            <span class="bbsBtn"><a href="javascript:fncInsert()" title="스케줄 등록 화면 호출">등록</a></span>
        </div>
        <!-- 버튼 End -->
        
        <!-- List Start -->
        <div class="bbsList">
            <table  summary="배치ID, 업무구분, Job이름(배치명), 스케줄번호, 스케줄명, 실행주기, 등록일자 순서로 스케줄 목록을 나타낸 표입니다.">
                ...
                <tbody>
                  <c:forEach var="list" items="${schdulList}">
                    <tr>
                        ...
                        <td align="left"  >
                            <a href="javascript:fncSelect('${list.batchId}', '${list.schdulNo}')" title="스케줄 상세정보 화면 호출"><c:out value="${list.schdulNm}" /></a>
                        </td>
