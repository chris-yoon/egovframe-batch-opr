...
<%
 /**
  * @Class Name : EgovCodeDplctCnfirm.jsp
  * @Description : 아이디중복확인
...
function fncheckCode(){
	if(fnCheckNotKorean(document.checkForm.checkCode.value) && fnCheck(document.checkForm.checkCode.value)){
		document.checkForm.action ="<c:url value='/sec/ram/EgovCodeDplctCnfirm.do'/>";
...
function fnReturnCode(){
    if (document.checkForm.usedCnt.value == 0){
	    opener.document.authorManage.authorCode.value = document.checkForm.resultCode.value;
        self.close();
    }else if (document.checkForm.usedCnt.value == 1){
        alert("이미 사용중인 권한코드입니다.");
        return;
    }else{
...
<div class="contsBody">
	<h2>권한코드중복확인</h2>
	<div class="location">등록 > <strong>권한코드중복확인</strong></div>
	
	<div class="search">
	<fieldset class="searchboxA">
		<legend>권한코드중복확인</legend>
		...
		<input type="text" name="checkCode" value="<c:out value="${checkCode}"/>" maxlength="20" tabindex="1" title="권한코드입력"/>                       
		<input type="image" class="searchbtn" title="조회" src="../../../images/egovframework/bopr/btn_search.gif" alt="검색" onclick="javascript:fncheckCode();"/>
	</fieldset>
	</div>
	...
	<fieldset class="searchboxA">
	    ...
	    <c:choose>
		    ...
		    <c:when test="${usedCnt eq 0}">
		        ${checkCode} 는 사용가능한 권한코드입니다.
		    </c:when>
		    <c:otherwise>
		        ${checkCode} 는 사용할수 없는 권한코드입니다.
		    </c:otherwise>
	    </c:choose>
	</fieldset>
    </div>
