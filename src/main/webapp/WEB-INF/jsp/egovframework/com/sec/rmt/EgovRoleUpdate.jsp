...
<div class="contsBody">
	<h2>롤 관리</h2>
	<div class="location">권한 관리 > 롤 관리 > <strong>수정</strong></div>

	<div class="Btn">
		<span class="bbsBtn"><a href="javascript:fncSelectRoleList()" title="롤관리 목록화면으로 이동">목록</a></span>
		
		<c:if test="${registerFlag == 'INSERT'}">
			<span class="bbsBtn"><a href="javascript:fncRoleInsert()" title="입력한 롤정보를 저장">등록</a></span>
		</c:if>
		
		<c:if test="${registerFlag == 'UPDATE'}">
			<span class="bbsBtn"><a href="javascript:fncRoleUpdate()" title="입력한 롤정보를 저장">수정</a></span>
			<span class="bbsBtn"><a href="javascript:fncRoleDelete()" title="해당 롤을 삭제">삭제</a></span>
		</c:if>
	</div>

	<div class="bbsDetail">
		<table  summary="롤코드, 롤명, 롤패턴, 롤타입, 롤인증순서 순서로 보여지는 롤관리 수정입니다.">
		<caption>롤관리 수정</caption>
		<colgroup>
			<col style="width:20%" >
		...
		<tr>
			<th><img src="<c:url value='/images/egovframework/bopr/blt4.gif' />" alt="필수입력" />롤  코드</th>
			<td>${roleManage.roleCode}</td>
		</tr>
		<tr>
			<th><img src="<c:url value='/images/egovframework/bopr/blt4.gif' />" alt="필수입력" />롤 명</th>
			<td>
				<label for="roleNm" class="disp_none">롤 명</label>
		...
		<tr>
			...
			<td>
				<label for="rolePtn" class="disp_none">롤 패턴</label>
		...
		<tr>
			...
			<td>
				<label for="roleTyp" class="disp_none">롤 타입</label>
