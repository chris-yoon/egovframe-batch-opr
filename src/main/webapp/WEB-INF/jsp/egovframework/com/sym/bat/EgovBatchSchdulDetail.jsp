...
function fn_egov_update_view(){
    ...
    varForm.action = "/com/sym/bat/getBatchSchdulForUpdate.do";
    varForm.submit();
}
/* ********************************************************
 * 삭제 처리
 ******************************************************** */
 function fn_egov_delete(){
        ...
        if(confirm("<spring:message code='common.delete.msg' />")){
            vForm.action = "/com/sym/bat/deleteBatchSchdul.do";
            vForm.submit();
        }
}
</script>
...
<form name="batchSchdulForm"  id="batchSchdulForm" action="<c:url value='/com/sym/bat/getBatchSchdul.do'/>" method="post">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" class="table-register" summary=" 배치스케줄 상세정보 테이블.">
      ...
      <tr> 
        <th height="23" class="required_text" scope="row" >파라미터 Input File</th>
        <td>
            <c:out value="${resultInfo.paramtrIn}" escapeXml="false" /> 
        </td>
      </tr> 
      
      <tr> 
        <th height="23" class="required_text" scope="row" >파라미터 Output File</th>
        <td>
            <c:out value="${resultInfo.paramtrOut}" escapeXml="false" /> 
        </td>
      </tr>       
    </table>
    <!--  줄간격조정  -->
    ...
    <tr> 
        <td>
            <span class="button">
            <a href="<c:url value='/com/sym/bat/getBatchSchdulForUpdate.do'>
                     </c:url>"
